...
export const Navigation = memo(function Navigation() {
  ...
  const toggleMobileMenu = () => {
    ...
    setProfileMenuOpen(false); // Close profile menu when mobile menu toggles
  };
  ...
  return (
    <>
      {/* Fixed Avatar - Always on top */}
      {isAuthenticated && (
        <div className="fixed top-2 right-2 z-[60] lg:hidden">
          <button
            className="w-10 h-10 rounded-full bg-tech-blue text-white flex items-center justify-center overflow-hidden focus:outline-none shadow-lg"
          ...
          {/* Mobile Profile Dropdown - Only show when mobile menu is closed */}
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-background/90 backdrop-blur-lg border-b border-border/50">
        ...
        <div className="flex items-center h-16 lg:h-20">
          ...
          {/* Desktop Profile/Sign In - Positioned on the right */}
              ...
              <div className="mt-6 pt-4 border-t border-border/50">
                {isAuthenticated ? (
                  <div className="flex items-center gap-3">
                    <span className="font-tech text-tech-dark">{user.username}</span>
                    <button
                      className="ml-auto px-3 py-2 rounded bg-tech-light hover:bg-red-100 text-red-600 font-tech font-medium"
                      onClick={logout}
                    >
                      Sign Out
                    </button>
                  ...
                  <Link
                    href="/user/login"
                    className="font-tech block w-full px-4 py-3 rounded-full font-bold transition bg-tech-blue hover:bg-tech-purple text-white shadow-lg text-center"
